...
fun deriveTones(primary: Color, dark: Boolean): AppTones {
    ...
    val secondaryH = (hsv.h + 30f) % 360f
    val tertiaryH = (hsv.h - 30f + 360f) % 360f
    val s2 = hsv.s * 0.55f
    val v2 = if (dark) 0.92f else 0.45f
    val secondary = hsvToColor(secondaryH, s2, v2)
    val tertiary = hsvToColor(tertiaryH, s2 * 0.8f, v2)
...
fun FlightTrackerTheme(
    ...
    val tinted = if (surfaceTint > 0f) {
        val t = surfaceTint.coerceIn(0f, 1f)
        val tint = tones.primary
        scheme.copy(
            background = lerp(scheme.background, tint, t),
            surface = lerp(scheme.surface, tint, t),
            surfaceVariant = lerp(scheme.surfaceVariant, tint, t * 1.4f)
        )
    } else {
        scheme
    }
